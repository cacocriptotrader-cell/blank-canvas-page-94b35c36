...
function PatrimonioPage() {
  ...
  const populateMockData = useCallback(() => {
    const mockAssets: Asset[] = [
      { id: Math.random().toString(36).slice(2), category: "Imóvel", description: "Apartamento — Pinheiros (SP)", currentValue: 850000 },
      { id: Math.random().toString(36).slice(2), category: "Veículo", description: "Honda Civic 2022", currentValue: 120000 },
      { id: Math.random().toString(36).slice(2), category: "Renda Fixa", description: "CDB Banco XP — 110% CDI", currentValue: 180000 },
      { id: Math.random().toString(36).slice(2), category: "Renda Variável", description: "Carteira de Ações B3", currentValue: 75000 },
      { id: Math.random().toString(36).slice(2), category: "Consórcio", description: "Consórcio imobiliário (cota)", currentValue: 32000 },
    ];
    const mockLiabilities: Liability[] = [
      { id: Math.random().toString(36).slice(2), category: "Financiamento Imobiliário", description: "Caixa SBPE — 30 anos", totalAmount: 600000, remainingBalance: 480000, interestRate: 11.5 },
      { id: Math.random().toString(36).slice(2), category: "Financiamento Veículo", description: "CDC Honda Civic", totalAmount: 90000, remainingBalance: 42000, interestRate: 18.9 },
    ];
    setAssets(mockAssets);
    setLiabilities(mockLiabilities);
  }, [setAssets, setLiabilities]);

  const ACCEPTED_EXT = [".pdf", ".dec", ".decrf", ".xml", ".txt"];
  ...
  const processFile = useCallback(
    (file: File | undefined | null) => {
      ...
      if (!accepted || file.size === 0) {
        const msg = "Erro ao ler arquivo. Formato não suportado ou corrompido.";
        setReadError(msg);
        toast.error(msg);
        return;
      }
  ...
  const simulate = useCallback(() => {
    setAnalyzing(true);
    setFileName("DIRPF_2025.pdf");
    setTimeout(() => {
      populateMockData();
      setAnalyzing(false);
      setReadSuccess(true);
      toast.success("Arquivo DIRPF_2025.pdf lido com sucesso");
    }, 1200);
  }, [populateMockData]);
          ...
          {analyzing ? (
            ...
            <>
              ...
              <p className="text-sm text-muted-foreground max-w-md">
                ...
                seus Bens, Direitos e Dívidas de forma <span className="text-foreground">100% segura e criptografada</span>.
              </p>

              <input
                ref={fileInputRef}
                type="file"
                accept=".pdf,.dec,.decrf,.xml,.txt,application/pdf"
              ...
              <div className="flex flex-wrap items-center justify-center gap-2 mt-2">
                ...
                <button
                  type="button"
                  onClick={() => simulate()}
                  className="px-4 py-2 rounded-xl text-primary-foreground text-sm font-medium inline-flex items-center gap-2"
                  style={{ background: "var(--gradient-primary)", boxShadow: "var(--shadow-glow)" }}
                >
                  <Sparkles className="h-4 w-4" /> Simular Extração
                </button>
                {readSuccess && (
