...
export const DIRPF_PARSER_VERSION = "dirpf-parser-no-worker-2026-05-15-01";
...
export async function parseDirpfPdf(file: File): Promise<ParsedDirpfPatrimony> {
  try {
    ...
    try {
      ...
      console.error("[DIRPF PDF] Falha ao carregar documento PDF sem worker", error);
    ...
    console.info("[DIRPF PDF] Texto extraído", { chars: text.length, pages: pdf.numPages });
